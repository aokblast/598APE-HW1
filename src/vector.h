...
public:
  ...
  inline Vector operator*(const int rhs) {
    return Vector(x * rhs, y * rhs, z * rhs);
  }
  inline Vector operator/(const double rhs) {
    return Vector(x / rhs, y / rhs, z / rhs);
  ...
  inline Vector cross(const Vector a) {
    return Vector(y * a.z - z * a.y, z * a.x - x * a.z, x * a.y - y * a.x);
  }
  inline double mag2() { return x * x + y * y + z * z; }
  inline double mag() { return sqrt(x * x + y * y + z * z); }
  inline double dot(const Vector a) { return x * a.x + y * a.y + z * a.z; }
